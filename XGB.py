...
def main():
    ...
    training_scores = []
    testing_scores = []
    learns = np.arange(0.1, 0.8, 0.1)
    for lea in learns:
        # param={'learning_rate':np.arange(0.1,0.6,0.1)}
        clf = XGBClassifier(learning_rate=lea, reg_alpha=0, min_child_weight=1, scale_pos_weight=1, n_estimators
        =30, max_depth=6, objective="multi:softmax", num_class=2)
        # clf = XGBClassifier( learning_rate=0.5, n_estimators=140,
        ...
        training_scores.append(recall_score(Y_train, y_pred, pos_label=0))

        #testing model
        y_tpred = clf.predict(X_test)
        testing_scores.append(recall_score(Y_test, y_tpred, pos_label=0))
    ...
    ax.plot(learns, training_scores, label="Training Score", color='r', linestyle='--')
    ax.plot(learns, testing_scores, label="Testing Score", color='b')
