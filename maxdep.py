...
def main():
    ...
    training_scores = []
    testing_scores = []
    learns = range(10, 150, 20)
    for lea in learns:
        # param={'learning_rate':np.arange(0.1,0.6,0.1)}
        clf = XGBClassifier(learning_rate=0.5, reg_alpha=0, min_child_weight=1, scale_pos_weight=1, n_estimators
        =lea, max_depth=6, objective="multi:softmax", num_class=2)
